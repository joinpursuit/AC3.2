...
var movies: [[String:Any]] = [
    ...
    [
        ...
        "year": 2014,
        "genre": "drama",
        "cast": ["Bradley Cooper", "Sienna Miller", "Kyle Gallner" , "bob"],
...
for i in 0..<movies.count {
    if let genre = movies[i]["genre"] as? String, movieTitle = movies[i]["name"] as? String {
        switch genre {
        case "animation":
            print("\(movieTitle)  \(genre)  \u{1F60A}")
        case "drama":
            print("\(movieTitle) \(genre) \u{1F631}")
        case "action":
            print("\(movieTitle)  \(genre)  \u{1F4A5}")
...
for i in 0..<movies.count {
    if let movieTitle = movies[i]["name"] as? String, genre = movies[i]["genre"] as? String, year = movies[i]["year"] as? Int, cast = movies[i]["cast"] as? [String], descrpt = movies[i]["description"] as? String {
        
        moviesByName[movieTitle] = movies[i]
    }
}

let sortedMovies = moviesByName.sort{ $0.0 < $1.0 } //sorts keys in ascending order
...
// when trying to use the same method as 5 it fails to print the movies that have the same key type like in the genres. A Solution to the problem is to creat a dictionary with and array of dictionaries. When not nil add the unwrapped elements to the movies that have the same key like genre. That way movies with the same genre or any other sub categoriez arent overwritten//
...
for i in 0..<movies.count {
    if let movieTitle = movies[i]["name"] as? String, genre = movies[i]["genre"] as? String, year = movies[i]["year"] as? Int, cast = movies[i]["cast"] as? [String], descrpt = movies[i]["description"] as? String {
        ...
        if moviesByYear[year] != nil {
            moviesArray = moviesByYear[year]!
            moviesArray.append(movies[i])
            moviesByYear[year] = moviesArray
        } else {
            //if key does not already exist in our moviesByYear dictionary
            moviesArray.append(movies[i])  //adding movie[i] to moviesArray
            moviesByYear[year] = moviesArray //add new key with value moviesArray in dict
        }
    }
}
let sortedYears = moviesByYear.sort{ $0.0 < $1.0}

for movie in sortedYears {
    print(movie)
    print("")
}
...
for i in 0..<movies.count {
    if let movieTitle = movies[i]["name"] as? String, genre = movies[i]["genre"] as? String, year = movies[i]["year"] as? Int, cast = movies[i]["cast"] as? [String], descrpt = movies[i]["description"] as? String {
        ...
        if moviesByGenre[genre] != nil {
            moviesArr = moviesByGenre[genre]!
            moviesArr.append(movies[i])
            moviesByGenre[genre] = moviesArr
        } else {
            //if key does not already exist in our moviesByYear dictionary
            moviesArr.append(movies[i])  //adding movie[i] to moviesArray
            moviesByGenre[genre] = moviesArr //add new key with value moviesArray in dict
        }
    }
}

let sortedGenre = moviesByGenre.sort{ $0.0 < $1.0 }
...
for i in 0..<movies.count {
    if let name = movies[i]["name"] as? String, year = movies[i]["year"] as? Int, cast = movies[i]["cast"] as? [String], genre = movies[i]["genre"] as? String, president = presidentsByYear[year] {
        ...
        var genreFirstLetter = genre[genre.startIndex]
        
        if genreFirstLetter == "a" || genreFirstLetter == "e" || genreFirstLetter == "i" || genreFirstLetter == "o" || genreFirstLetter == "u" {
            print("\(name) came out in \(year). It was an \(genre) starring \(actors)")
        } else {
            print("\(name) came out in \(year). It was a \(genre) starring \(actors)")
        }
