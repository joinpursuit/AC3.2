...
print(movies[0]["name"])                                        //this gives you optional bind it before

// 2. Print a list of all movie names, preferably on one line.



for movie in movies {  //movie is dictionary movie is array
    
    
    if let name = movie["name"]{  // so you should iterate inside the dictionory
...
for i in 0..< movies.count{
    ...
    if let name = movies[i]["name"] as? String, year = movies[i]["year"] as? Int, genre = movies[i]
...
//for i in 0..<movies.count {                                                //we added var here later
...
//
//        if cast.count > 1 {
//            cast.insert("and", atIndex:cast.count - 2)
//
//        }
////       var casts = ""    this didn't work
//
////
