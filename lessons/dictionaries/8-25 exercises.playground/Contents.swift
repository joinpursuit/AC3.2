...
var citiesDict: [String:String] = ["USA" : "Washington DC", "France" : "Paris", "Canada" : "Ottawa"]
...
for (country,capital) in citiesDict {
    if capital == "Washington DC" {
        citiesDict.updateValue("DC in Spanish", forKey: "USA")
    }
    if capital == "Dhaka" {
        citiesDict.updateValue("Dhaka in Bengali", forKey: "Bangladesh")
    }
    if capital == "Santo Domingo" {
        citiesDict.updateValue("Santo Domingo in English", forKey: "Dominican Republic")
    }
}
print(citiesDict)
//2)
...
var sumOfOneToSix = sum - someDict["productUpToSeven"]! - someDict["Seven"]!
print(sumOfOneToSix)
...
print(scoresDict["John Steinbeck"])
scoresDict["Erik Larson"] = 9.2
print(scoresDict)
if scoresDict["John Krakaur"] > scoresDict["Mark Twain"] {
    print("John has the highest score")
}
else {
    print("Mark has the highest score")
}
