...
func totalWithTip (cost: Double, tipPercent: Double) -> Double {
    let totalCost = cost + cost * tipPercentage
    return totalCost
}


let myFinalCost = totalWithTip(itemCost, tipPercent: tipPercentage) //Fill in the arguments
...
func totalWithTipAndTax (cost: Double, tipPercent: Double, taxPercent: Double) -> Double {
    let finalCost = (cost + cost * taxPercent) * (1 + tipPercent)
    return finalCost
}

let myFinalCostWithTipAndTax = totalWithTipAndTax(itemCost, tipPercent: tipPercentage, taxPercent: taxPercentage) //Fill in the arguments
...
func returnMax(intArray: [Int]) -> Int {
    var max = 0
...
print("The largest Int in myArray is \(returnMax(myArray))")
...
if isEven(returnMax(myArray)) {
    print("The largest Int of myArray is Even")
}
else {
    print("The largest Int of myArray is Odd")
}
...
func returnNumOfCharsInString (str: String) -> Int {
...
print(returnNumOfCharsInString(myString))
...
func matchingCharCounter (str: String, char: Character) -> Int {
    var counter = 0
...
let charCount = matchingCharCounter(FiveCString, char: targetCharacter)
print(charCount)
...
func matchingMultipleCharCounter (str: String, charArray: [Character]) -> Int {
    var counter = 0
    
    for checkingChar in charArray {
        
        for character in str.characters {
        
            if checkingChar == character {
            counter += 1
            }
        }
    }
...
let sampleAnswer = matchingMultipleCharCounter(FiveDString, charArray: targetCharacters)
...
//
//func uniqueNumCounter (intArray: [Int]) -> Int {
//    
//    var num: Int
//    var numArray = [Int]()
//    
...
//
//
//let newCounter = uniqueNumCounter(inputArray)
//print(newCounter)
