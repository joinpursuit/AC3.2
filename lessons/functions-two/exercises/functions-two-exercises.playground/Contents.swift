...
func totalWithTip(Cost x: Double, Tip y: Double) -> Double {
    let total = x + (x * y)
    print(total)
    return total
}

let myFinalCost = totalWithTip(Cost: Double(itemCost), Tip: tipPercentage) //Fill in the arguments
...
func totalWithTipAndTax(Cost x: Double, Tip y: Double, Tax z: Double) -> Double {
    let totalCostWithTax = x + (x * y)
    let totalCostWithTaxAndTip = Double(totalCostWithTax) + (Double(totalCostWithTax) * z)
    print(totalCostWithTaxAndTip)
    return totalCostWithTaxAndTip
}


let myFinalCostWithTipAndTax = totalWithTipAndTax(Cost: Double(itemCost), Tip: tipPercentage, Tax: taxPercentage) //Fill in the arguments
...
func largestIntInArray (x: [Int]) -> Int {
    var temp = x[0]
    for element in x {
        if temp < element {
            temp = element
...
print("The largest Int in myArray is \(largestIntInArray(myArray)).")
...
if largestIntInArray(myArray) % 2 == 0 {
    print("The largest Int in myArray is EVEN and has a value of \(evenOrOdd(largestIntInArray(myArray)))")
} else {
    print("The largest Int in myArray is ODD and has a value of \(evenOrOdd(largestIntInArray(myArray)))")
}
...
func numberOfCharacters (x: String) -> Int {
...
func instancesOfSpecificCharacter (x: String, y: Character) -> Int {
    var counter = 0
...
instancesOfSpecificCharacter(fiveCString, y: targetCharacter)
...
func targetCharacterCounter (x: String, y: [Character]) -> Int {
    var counter = 0
    for i in x.characters {
        for j in y {
            if i == j {
...
targetCharacterCounter(fiveDString, y: targetCharacters)
...
func binaryIntoDecimal (x: [Int]) -> Double {
    var binarySum : Double = 0.0
    for var i = binaryArray.count - 1; i >= 0; i -= 1 {
        if x[i] == 1 {
            binarySum += pow(2.0, Double(i))
        }
    }
    return binarySum
}

binaryIntoDecimal(binaryArray)


//func binaryIntoDecimal (x : [Int]) -> Double {
//    var binarySum : Double = 0.0
//    var reverseArray = Array(binaryArray.reverse())
//    for i in 0..<reverseArray.count - 1 {
//        if x[i] == 1 {
//            binarySum += pow(2.0, Double(i))
//        }
//    }
