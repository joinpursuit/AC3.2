...
//
//  Created by Kareem James on 9/6/16.
...
for movieDict in movies {
    if let name = movieDict["name"]as? String, year = movieDict["year"] as? Int, cast = movieDict["cast"] as? [String], genre = movieDict["genre"] as? String, description = movieDict["description"] as? String  {
        let movie = Movie() // making an instance of the class movie that we created on the other page
        
        
        for movieStar in cast {
            // make an empty Actor
            let actor = Actor(breakoutYear: 1900, breakoutRole: "The Movie", name: "Actor", born: 1900, died: nil)
...
for president in presidentData {
    let presidentArray = president.componentsSeparatedByString(", ")
...
let presidentsByYearArray = Array(presidentsByYear.keys)
print(presidentsByYearArray)
...
//let ronaldReagan = President(entered: 1989, left: 1993, name: "Ronald Reagan", born: 1920, died: 2004)
...
//ronaldReagan.yearsInOffice = 10
////io = ronaldReagan.yearsInOffice
//
//
//dump(ronaldReagan)
//
